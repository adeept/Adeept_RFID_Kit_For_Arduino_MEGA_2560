...
const int NRSTPD = 9;
const int relayPin = 10; //the base of the transistor attach to
//MF522 command bits
...
void loop()
{
    ...
    if (status == MI_OK)
    {
        ...
        uchar* id = serNum;
        if( id[0]==0xD5 && id[1]==0x6B && id[2]==0x63 && id[3]==0x76 ) {
           digitalWrite(relayPin, HIGH); //drive relay closure conduction
           delay(1000); //wait for a second

           digitalWrite(relayPin, LOW); //drive the relay is closed off
           delay(1000); //wait for a second  C089292B
        } else if(id[0]==0xC0 && id[1]==0x89 && id[2]==0x29 && id[3]==0x2B) {
              digitalWrite(relayPin, HIGH); //drive relay closure conduction
              delay(1000); //wait for a second
